...
def get_args_parser():
    ...
    parser.add_argument("--steps-to-predict", type=int, default=2)
    parser.add_argument("--random-trajectory-sampling", type=bool, default=True)
...
def train_one_epoch(
    ...
    for batch_idx, batch_data in enumerate(data_loader):
        batch_size, n_nodes, _ = batch_data[0].size()
        batch = torch.arange(0, batch_size).repeat_interleave(n_nodes).long()
...
def main(args):
    ...
    model_save_path = f"{save_dir_path}/{args.dataset_name}_best_model.pth"
    torch.save(model.state_dict(), model_save_path)
    print(f"Model saved to {model_save_path}")

    dataset_save_path = f"{save_dir_path}/{args.dataset_name}_dataset"
    os.makedirs(dataset_save_path, exist_ok=True)
    for prefix in dataset_test.GROUND_TRUTH_FILE_PREFIXES:
